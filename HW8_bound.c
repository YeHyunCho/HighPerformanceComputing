...
int main(int argc, char* argv[])
{
   ...
   if (pid==0) printf("%dx%d procssors are used\n", inp, jnp);
   ...
   // Use coord[2] -> top, bottom, leftmost, rightmost

   if (coord[0] == 0)
      for (i=0; i<local_N; i++)
         local_A[0][i] = (coord[1]*local_N) + i;
   if (coord[0] == (inp-1))
      for (i=0; i<local_N; i++)
         local_A[local_M-1][i] = (M-1) + (coord[1]*local_N) + i;
   if (coord[1] == 0)
      for (i=0; i<local_M; i++)
